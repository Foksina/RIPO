# object_identification_module.py

import cv2
import numpy as np

class ObjectIdentificationModule:
    def __init__(self):
        # Inicjalizacja modułu identyfikacji obiektów
        self.net = cv2.dnn.readNet('yolov3.weights', 'yolov3.cfg') # Wczytanie modelu YOLOv3
        self.classes = []
        with open('coco.names', 'r') as f: # Wczytanie nazw klas
            self.classes = f.read().splitlines()
        
        self.detected_objects = [] 


    def identify_objects(self, frame):
        # Identyfikacja obiektów - wykrywanie samochodów, pieszych i rowerzystów za pomocą modelu YOLOv3
        height, width, _ = frame.shape

        # Przygotowanie wejścia do sieci neuronowej
        blob = cv2.dnn.blobFromImage(frame, 1 / 255.0, (416, 416), swapRB=True, crop=False)
        self.net.setInput(blob)

        # Przejście przez sieć neuronową i uzyskanie wyjść
        output_layers_names = self.net.getUnconnectedOutLayersNames()
        layer_outputs = self.net.forward(output_layers_names)

        # Przetwarzanie wyjść i wykrywanie obiektów
        boxes = []
        confidences = []
        class_ids = []

        self.detected_objects = []

        for output in layer_outputs:
            for detection in output:
                scores = detection[5:]
                class_id = np.argmax(scores)
                confidence = scores[class_id]
                if confidence > 0.5 and (class_id in [0, 2, 3]): # Indeks 0: "person", 2: "car", 3: "bicycle"
                    center_x = int(detection[0] * width)
                    center_y = int(detection[1] * height)
                    w = int(detection[2] * width)
                    h = int(detection[3] * height)
                    x = int(center_x - w / 2)
                    y = int(center_y - h / 2)
                    boxes.append([x, y, w, h])
                    confidences.append(float(confidence))
                    class_ids.append(class_id)
                    self.detected_objects.append((x, y, w, h, class_id))

        # Wykorzystanie Non-Maximum Suppression w celu usunięcia duplikatów
        indexes = cv2.dnn.NMSBoxes(boxes, confidences, 0.5, 0.4)

        # Rysowanie prostokątów wokół wykrytych obiektów
        for i in range(len(boxes)):
            if i in indexes:
                x, y, w, h = boxes[i]
                label = self.classes[class_ids[i]]
                if class_ids[i] == 3: # fioletowy dla rowerzystow
                    color = (182, 0, 178)
                elif class_ids[i] == 2: # niebieski dla samochodów
                    color = (255, 0, 0)
                else:
                    color = (1, 98, 255) # pomaranczowy dla pieszych
                cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
                cv2.putText(frame, label, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

        return frame
    
    def get_detected_objects(self):
        return self.detected_objects
